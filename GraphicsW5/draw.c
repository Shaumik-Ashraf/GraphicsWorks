...
void draw_polygons( struct matrix *polygons, screen s, color c ) {
  ...
  z = (polygons->lastcol)/3;
  for(y=0;y<z;y++){
    x0 = polygons->m[0][y*3];
    y0 = polygons->m[1][y*3];
    z0 = polygons->m[2][y*3];
    x1 = polygons->m[0][y*3+1];
    y1 = polygons->m[1][y*3+1];
    z1 = polygons->m[2][y*3+1];
    x2 = polygons->m[0][y*3+2];
    y2 = polygons->m[1][y*3+2];
    z2 = polygons->m[2][y*3+2];
    double q1[3] = {x1-x0, y1-y0, z1-z0};
    double q2[3] = {x2-x0, y2-y0, z2-z0};
    double q3[3] = {(q1[1]*q2[2])-(q1[2]*q2[1]),
		    (q1[2]*q2[0])-(q1[0]*q2[2]),
		    (q1[0]*q2[1])-(q1[1]*q2[0])};
    double q4 = sqrt((q3[0]*q3[0])+(q3[1]*q3[1])+(q3[2]*q3[2]));
    double q5 = (q3[0]*0+q3[1]*0+q3[2]*-1)/(q4*1);
    if(q5 >= 0){
...
void add_sphere( struct matrix * points, 
  ...
  for(lat = latStart;lat<latStop;lat++){
    for(longt = longStart;longt*2<longStop;longt++){
      x = lat * num_steps + longt;
      x2 = x + num_steps;
      if (lat == latStop-1) {				
	x2 = latStart * num_steps + longt;
      }
      add_polygon(points, temp->m[0][x],
		  temp->m[1][x],
		  temp->m[2][x],
		  temp->m[0][x2 + 1],
		  temp->m[1][x2 + 1],
...
int do_i_draw_it(double x0, double y0, double z0,
  ...
  //set view vector
  view_vector[0] = view_x;
  view_vector[1] = view_y;
  view_vector[2] = view_z;
  ...
  surface_normal[0] = v0_y*v1_z - v0_z*v1_y;
  surface_normal[1] = v0_z*v1_x - v0_x*v1_z;
  surface_normal[2] = v0_x*v1_y - v0_y*v1_x;
  
  //compute dot product
  for(i=0; i<3; i++) {
    dot_prod += (surface_normal[i]*view_vector[i]);
  }
